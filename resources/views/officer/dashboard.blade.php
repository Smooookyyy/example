...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        ...
        <div class="mt-8">
            ...
            @php
            $rejectedFormsHHMD = \App\Models\hhmdsaved::where('submitted_by', Auth::guard('officer')->id())
            ...
            @php
            $rejectedFormsWTMD = \App\Models\wtmdsaved::where('submitted_by', Auth::guard('officer')->id())
            ->where('status', 'rejected')
            ->orderBy('reviewed_at', 'desc')
            ->get();
            @endphp

            @if($rejectedFormsHHMD->count() > 0 || $rejectedFormsWTMD->count() > 0)
            <div class="overflow-x-auto">
                <table class="min-w-full bg-white border border-gray-300">
                    ...
                    <tbody>
                        @foreach($rejectedFormsHHMD as $form)
                        ...
                        @endforeach
                        @foreach($rejectedFormsWTMD as $form)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 border-b">{{ $form->testDateTime->format('d/m/Y H:i') }}</td>
                            <td class="px-6 py-4 border-b">{{ $form->location }}</td>
                            <td class="px-6 py-4 border-b text-red-600">
